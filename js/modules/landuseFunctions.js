...
import { getDescriptions, getNames } from "./getNamesAndDescriptions.js";
...
export async function getLanduseData(scenario) {
  await fetchData(`/catchment/Scenarios/${scenario}/TxtInOut/landuse.lum`)
    .then(async data => {
      ...
      const landuseTypes = await getNames(cleanLanduseData);
...
export async function getGrassedWw(scenario) {
  ...
  await fetchData(`/catchment/Scenarios/${scenario}/TxtInOut/grassedww.str`)
    .then(async function (data) {
      const cleanNData = cleanTsvSwatFiles(data);

      const nNames = await getNames(cleanNData);

      const manNDescriptions = await getDescriptions(cleanNData)
...
export async function getFilterStrip(scenario) {
  await fetchData(`/catchment/Scenarios/${scenario}/TxtInOut/filterstrip.str`)
    .then(async function (data) {
      const cleanTileData = cleanTsvSwatFiles(data);
      const tileNames = await getNames(cleanTileData);
      const tileDescriptions = await getDescriptions(cleanTileData)
...
export async function getCurveNumer(scenario) {
  ...
  await fetchData(`/catchment/Scenarios/${scenario}/TxtInOut/cntable.lum`)
    .then(async function (data) {
      const cleanCnData = cleanTsvSwatFiles(data);
      const cnNames = await getNames(cleanCnData);
      const cnDescriptions = await getDescriptions(cleanCnData)
...
export async function getConsPractice(scenario) {
  ...
  await fetchData(`/catchment/Scenarios/${scenario}/TxtInOut/cons_practice.lum`)
    .then(async function (data) {
      const cleanConsData = cleanTsvSwatFiles(data);
      const consNames = await getNames(cleanConsData);
      const consDescriptions = await getDescriptions(cleanConsData)
...
export async function getUrbanList(scenario) {
  ...
  await fetchData(`/catchment/Scenarios/${scenario}/TxtInOut/urban.urb`)
    .then(async function (data) {
      ...
      const urbanNames = await getNames(cleanUrban);
      const urbanDescription = await getDescriptions(cleanUrban)
